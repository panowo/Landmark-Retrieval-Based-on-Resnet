...
class FineTuneModel(nn.Module):
    def __init__(self, num_ffftrs,original_model, arch, num_classes):
...
def main():
    ...
    if args.finetune:
        ...
        save_model = FineTune(original_model, args.arch, 45)
    else:
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            save_model.load_state_dict(checkpoint['state_dict'],strict=False)
            print("=> loaded checkpoint '{}' (epoch {})"
                  .format(args.evaluate, checkpoint['epoch']))
        else:
            print("=> no checkpoint found at '{}'".format(args.resume))
    
    num_ffftrs=original_model.fc.in_features
    model=FineTuneModel(num_ffftrs, save_model, args.arch, 11)
    model = torch.nn.DataParallel(model).cuda()
