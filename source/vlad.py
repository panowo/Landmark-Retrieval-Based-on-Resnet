...
class VladPrediction(object):
	def __init__(self, dataset, path, query):
		...
		self.name = DIR + os.path.sep + dataset.dataset_name +'_2_clusters_and_vlad_descriptors.pickle'
	...
	def get_SIFT_descriptors(self, dataset):
		...
		set_trace()
		# descriptors = np.array(itertools.chain.from_iterable(descriptors))
	...
	def get_clusters_vlad_descriptors(self, dataset, k=2):
		'''
