...
parser.add_argument('--test_dataset', type=str, help='Path to your test dataset')
...
parser.add_argument('--mode', type=str, help='train: Extract feature vectors from given dataset, \n'
                                                ...
                                                'divide_dataset: Dividing dataset into train, val, test and index from dataset\n'
                                                'create_dataset_csv: Creating csv for dataset\n'
                                                'cbir: Print 5 most similar photo for query\n'
...
parser.add_argument('--distance', type=str, help='L1: Manhattan Distance \n'
                                                ...
                                                'L3: cosine similiarity\n', required=False, default='L1')
parser.add_argument('--method', type=str, help='one: origin \n'
...
if __name__ == "__main__":
    ...
    if args.mode == 'train':
        ...
        if args.model == 'resnet':
            ...
            vlad_class = VladPrediction(vlad_dataset, args.dataset, args.query)
            if args.method =='one':
                calculate_map_vlad_metric(vlad_dataset, test_dataset,vlad_class,args.distance)
            else:
                calculate_new_map_vlad_metric(vlad_dataset, test_dataset,vlad_class,args.distance)
        ...
        if args.model == 'resnet':
            ...
            vectors = extractor.feature_vectors(dataset)
            start_t=time.time()
            save_five_nearest_photo(extractor.compute_query_vector(args.query), vectors, args.distance, args.method)
